      ...
      do K=2,nz
        if (CS%use_GL90_N2) then
          ...
          if (kdgl90_use_vert_struct) then
            a_cpl_gl90(I,K) = a_cpl_gl90(I,K) * 0.5 * ( VarMix%kdgl90_struct(i,j,k-1) + VarMix%kdgl90_struct(i+1,j,k-1) )
          endif
        endif
    ...
    do i=is,ie; if (do_i(i)) then
      ...
      do K=2,nz
        if (CS%use_GL90_N2) then
          ...
          if (kdgl90_use_vert_struct) then
            a_cpl_gl90(i,K) = a_cpl_gl90(i,K) * 0.5 * ( VarMix%kdgl90_struct(i,j,k-1) + VarMix%kdgl90_struct(i,j+1,k-1) )
          endif
        endif
        ! botfn determines when a point is within the influence of the GL90 bottom boundary layer,
        ! going from 1 at the bottom to 0 in the interior.
